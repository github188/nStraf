package cn.grgbanking.feeltm.monthMission.dao;
...
@Repository("monthMissionDao")
public class MonthMissionDao extends BaseDao<MonthMission> {
	public Page getPage(String groupname,String username,
			int pageNum, int pageSize, String _monthDate, String _examRank, int querylevel) {
		String hql ="";
		hql = "from MonthMission where 1=1 ";
		...
		if(_examRank!=null  && !_examRank.equals("全选")){
			hql += " and examRank like '%"+_examRank.trim()+"%'";
		}
		if(querylevel == 1)
		{
		...
		hql += " order by monthDate desc, to_number(examScore) desc, groupName desc";
	...
	public boolean checkMonthDate(String _monthDate) throws Exception{
		boolean flag = false;
		String hql = "from MonthMission where 1=1 ";
		hql += " and monthDate like '%" + _monthDate.trim()+"%'";
		List list = getHibernateTemplate().find(hql);
		if(list.size()!=0)
		{
	...
	public boolean remove(String[] ids){
		boolean flag=false;
		List<MonthMission> bs=new ArrayList<MonthMission>();
		for (int j = 0; j < ids.length; j++) {	
				bs.add(new MonthMission(ids[j]));
			}
		try{
...
//		names.add(0, "全选");
	...
	public List<String> getIdByMonthDate(String _monthDate){
		String query="select mm.id from MonthMission mm where 1=1 and monthDate like '%" + _monthDate + "%'";
