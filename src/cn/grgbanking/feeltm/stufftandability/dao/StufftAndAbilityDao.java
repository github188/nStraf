package cn.grgbanking.feeltm.stufftandability.dao;
...
@Repository("stufftAndAbilityDao")
public class StufftAndAbilityDao extends BaseDao<StufftAndAbility> {
	public Page getPage(String groupname,String username,
		...
		hql = "select new cn.grgbanking.feeltm.stufftandability.domain.Perfor(pno,id,rownum,month_date,user_id,group_name,subtotal_s,subtotal_d,subtotal_m,update_man,modify_date) from StufftAndAbility where 1=1 ";
	...
	public  String  getNextNo(){
		String str=null;
		List list=this.getHibernateTemplate().find("select max(p.pno) from StufftAndAbility p");
	...
	public List<String> getAllNames(){
	 ...
	 public boolean upEdit_lock(String month_date){   
	        try {   
	            ...
	            String hql = "update StufftAndAbility set edit_lock = 1 where month_date like '%"+month_date.trim()+"%'"; 
		...
		public String getHeadmanAddValue(String username,String month_date){
			...
			String hql="select count(y.id) from Instance y,SysUser k where y.embracer_man like '%"+username.trim()+"%' and y.create_man = k.username and  to_char(create_date,'yyyy-MM') like '%"+month_date.trim()+"%' and (k.level = 1 or k.level = 0)";
			System.out.println(hql);
			List list=this.getHibernateTemplate().find(hql);
			String str=null;
			if(list!=null&&list.size()>=0){
		...
		public String getFellowValue(String username,String month_date){
			...
			String hql="select count(y.id) from Instance y,SysUser k where y.embracer_man like '%"+username.trim()+"%' and y.create_man = k.username and  to_char(create_date,'yyyy-MM') like '%"+month_date.trim()+"%' and (k.level = 2)";
			System.out.println(hql);
			List list=this.getHibernateTemplate().find(hql);
			String str=null;
			if(list!=null&&list.size()>=0){
...
//		String hql="select new cn.grgbanking.feeltm.report.domain.DayReportStatic(t.startDate,to_char(t.startDate,'day'),count(distinct(t.startDate||t.username)),sum(t.subtotal)) from WorkReport t group by t.startDate having to_char(t.startDate,'yyyy-MM')=? order by t.startDate desc ";
	...
	public String getUseredit_lock(String month_date){
		...
		String hql="select edit_lock from StufftAndAbility where month_date like '%"+month_date.trim()+"%'";
		List list=this.getHibernateTemplate().find(hql);
		String str=null;
		if(list!=null&&list.size()>=0){
			str=list.get(0).toString();		     
	...
	public boolean getUserMonth_date(String month_date){
		...
		String hql="from StufftAndAbility where month_date like '%"+month_date.trim()+"%'";
		List list=this.getHibernateTemplate().find(hql);
		boolean str=false;
		if(list!=null&&list.size()>0){
			str=true;		     
	...
	public List<StufftAndAbility> selectStufftAndAbility(String pno){
		String query="from StufftAndAbility report where report.pno=?";
		List<StufftAndAbility> reports=(List<StufftAndAbility>)this.getHibernateTemplate().find(query, new Object[]{pno});
