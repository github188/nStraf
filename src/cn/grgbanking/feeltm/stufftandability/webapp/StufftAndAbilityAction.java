package cn.grgbanking.feeltm.stufftandability.webapp;
...
import cn.grgbanking.feeltm.stufftandability.service.StufftAndAbilityService;
...
public class StufftAndAbilityAction extends BaseAction{
	private static final long serialVersionUID = 1L;
	private StufftAndAbilityService stufftAndAbilityService;
	
	private StufftAndAbility stufftandability;
//	private String status;
    ...
    private List<StufftAndAbility> StufftAndAbilityList;
	...
	public String up() throws Exception {
		...
		stufftandability=stufftAndAbilityService.getCaseById(ids);
		if(stufftandability.getEdit_lock() == 1)
		{
		...
		if(stufftandability.getUser_id().equals(modify_man) && !stufftandability.getUser_id().equals("冯天桂"))
		{
			MsgBox msgBox = new MsgBox(request,
		...
		if(!stufftandability.getGroup_name().equals(userModel.getGroupName())&& !stufftandability.getUser_id().equals("冯天桂"))
		{
			MsgBox msgBox = new MsgBox(request,
					getText("operInfoform.updatefaile",new String[]{"你没有权限提交该记录"}));
			addActionMessage(getText("operInfoform.updatefaile"));
		...
		try {
			stufftandability.setEdit_lock(1);
			flag=stufftAndAbilityService.update(stufftandability);
			if (flag == true){
				MsgBox msgBox = new MsgBox(request,
					getText("提交成功"));
				msgBox.setButtonType(MsgBox.BUTTON_CLOSE);
	...
	public String save() throws Exception {     //保存suggestion概况的数据
		...
		try {
			...
			//	String[] usergroup=user_id.trim().split("、");
				...
				oldmonth_date = stufftAndAbilityService.getUserMonth_date(stufftandability.getMonth_date());
				if(oldmonth_date == true)
				{
					MsgBox msgBox1 = new MsgBox(request,getText("operInfoform.updatefaile",new String[]{"该月度素质与能力已创建，新增失败"}));
				...
				unames=stufftAndAbilityService.getAllNames();
			
				for(String mm:unames){
					no = stufftAndAbilityService.getNextNo();
					stufftandability.setPno(no);
					stufftandability.setUpdate_man(userModel.getUsername());	
					stufftandability.setGroup_name(stufftAndAbilityService.getUserGroup_name(mm.trim()));
					stufftandability.setUser_id(mm.trim());

					SimpleDateFormat f=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					stufftandability.setModify_date(f.format(new Date()));
					//performance.setMonth_date(month_date.trim());
					flag=stufftAndAbilityService.add(stufftandability);
				}
			if (flag == true) {
		...
		try {
			...
			if (sids != null) {
				for (int i = 0; i < sids.length; i++) {
					StufftAndAbility temp = stufftAndAbilityService.getCaseById(sids[i]);
					stufftAndAbilityService.delete(temp);
					iCount++;
				}
			}
	...
	public String edit() throws Exception {
		...
		try {
			...
			String oldUsername=stufftandability.getUser_id();
			String oldGname=stufftandability.getGroup_name();
			String oldMonth=stufftandability.getMonth_date();
			String userGname = userModel.getGroupName();
			int headmanLevel = userModel.getLevel();
			double oldeditlock = stufftandability.getEdit_lock();
			...
			if(0 == headmanLevel){
				if(stufftAndAbilityService.getUserlevel(oldUsername).equals("1"))
				{
	...
	public String update() throws Exception {
		...
		UserModel userModel = (UserModel) request.getSession().getAttribute(
				Constants.LOGIN_USER_KEY);
		modify_man=userModel.getUsername();
		StufftAndAbility oldSuggestion=stufftAndAbilityService.getCaseById(stufftandability.getId());
		...
		try {
			stufftandability.setEdit_lock(oldeditlock);
			String ano=oldSuggestion.getPno();
			stufftandability.setUser_id(oldUsername);
			stufftandability.setGroup_name(oldGname);
			stufftandability.setMonth_date(oldMonth);
			SimpleDateFormat f=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			stufftandability.setModify_date(f.format(new Date()));
			stufftandability.setUpdate_man(modify_man);
			stufftandability.setEdit_lock(oldEdit_lock);
			stufftandability.setPno(ano);
		//	stufftAndAbility.setFirstedit("已修改");
			boolean flag=stufftAndAbilityService.update(stufftandability);
			if (flag == true) {
				MsgBox msgBox = new MsgBox(request,
	...
	public String query() throws Exception {
		...
		try {			
			...
			Page page = stufftAndAbilityService.getPage(groupname, username,pageNum, pageSize, month);
	...
	public String queryNames() throws Exception{
		...
		PrintWriter out=response.getWriter();
		List<Object> list=stufftAndAbilityService.getNames(groupName);
	...
	public StufftAndAbilityService getStufftAndAbilityService() {
		return stufftAndAbilityService;
	}

	public void setStufftAndAbilityService(
			StufftAndAbilityService stufftAndAbilityService) {
		this.stufftAndAbilityService = stufftAndAbilityService;
	}

	public List<StufftAndAbility> getStufftAndAbilityList() {
		return StufftAndAbilityList;
	}

	public void setStufftAndAbilityList(List<StufftAndAbility> stufftAndAbilityList) {
		StufftAndAbilityList = stufftAndAbilityList;
	}
	...
	public StufftAndAbility getStufftandability() {
		return stufftandability;
	}

	public void setStufftandability(StufftAndAbility stufftandability) {
		this.stufftandability = stufftandability;
	}
