...
import cn.grgbanking.feeltm.prj.service.PrjPartService;
...
public class PrjPartAction extends BaseAction{
	private PrjPartService prjPartService;
	private String prjName;
	private String versionNO;
	private PrjPart prjparams;
	...
	public String save() throws Exception {
		try {
			boolean flag = false;
			flag=prjPartService.add(prjparams);
			if (flag == true) {
		...
		try {
			...
			if (sids != null) {
				for (int i = 0; i < sids.length; i++) {
					PrjPart temp = prjPartService.getPrjPartById(sids[i]);
					prjPartService.delete(temp);
					iCount++;
				}
			}
	...
	public String edit() throws Exception {
		...
		try {
			String ids = request.getParameter("ids");
			prjparams=prjPartService.getPrjPartById(ids);
		} catch (Exception e) {
			e.printStackTrace();
			SysLog.error(e);
		}
	...
	public String update() throws Exception {
		try {
			...
			boolean flag=prjPartService.update(prjparams);
			if (flag == true) {
				MsgBox msgBox = new MsgBox(request,
	...
	public String query() throws Exception {
		...
		try {
			...
			Page page = prjPartService.getPage(prjType1,prjName,month,pageNum, pageSize);
	...
	public String checkExist(){
		...
		String staticMonth=request.getParameter("month");
		try {
			PrintWriter out=response.getWriter();
			boolean flag=true;
			if(!prjPartService.isExist(prjName1, staticMonth)){
				flag=false;
			}
	...
	public PrjPartService getPrjPartService() {
		return prjPartService;
	}
	public void setPrjPartService(PrjPartService prjPartService) {
		this.prjPartService = prjPartService;
	}
	public PrjPart getPrjparams() {
		return prjparams;
	}
	public void setPrjparams(PrjPart prjparams) {
