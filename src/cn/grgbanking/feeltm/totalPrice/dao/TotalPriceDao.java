package cn.grgbanking.feeltm.totalPrice.dao;
...
public class TotalPriceDao extends BaseDao<TotalPrice> {
	...
	public boolean checkMonthDate(String _monthDate) throws Exception{
		boolean flag = false;
		String hql = "from TotalPrice where 1=1 ";
		hql += " and monthDate like '%" + _monthDate.trim()+"%'";
		List list = getHibernateTemplate().find(hql);
		if(list.size()!=0)
		{
	...
	public boolean remove(String[] ids){
		boolean flag=false;
		List<TotalPrice> bs=new ArrayList<TotalPrice>();
		for (int j = 0; j < ids.length; j++) {	
				bs.add(new TotalPrice(ids[j]));
			}
		try{
...
//		names.add(0, "全选");
	...
	public List<String> getIdByTotalPrice(String _monthDate){
		String query="select mm.id from TotalPrice mm where 1=1 and monthDate like '%" + _monthDate + "%'";
