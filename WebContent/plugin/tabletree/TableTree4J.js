...
function addToClosedGridTreeNode(nodeId){
	$closedGridTreeNodes+=","+nodeId;
	if($closedGridTreeNodes.indexOf(",")==0){
		$closedGridTreeNodes=$closedGridTreeNodes.substring(1);
	}
}

function removeFromGridTreeNode(nodeId){
	var idArray=$closedGridTreeNodes.split(",");
	var newStr="";
...
function clickNode(nodeid,tree){
	...
	if(clickNode.booleanLeaf==false){
		...
		if(clickNode.booleanOpen==true){
			...
			if($closedGridTreeNodes!=undefined){
				addToClosedGridTreeNode(nodeid);
			}
			//cookie
