...
window.onload = function() {
	...
	try {
		...
		for(;i<=1000;i++) {
			...
			var r =	new feelview.TransformXmlXsl(urlXml,urlXsl,mycallbackHtml);
			r.transform(renderParam); 
		}
		//演示二: 发请求(异步)
		//var xmlhttp = new feelview.XMLHttpRequest("POST",urlXml,true,mycallbackXml);
		//xmlhttp.send("Body");
		
		//演示三: 发请求(同步)
		//xmlhttp	 = new feelview.XMLHttpRequest("GET",urlXml,false);
...
*/
// namespace object
if(typeof(feelview)=='undefined') feelview = {};
/**
...
**/
feelview.XMLHttpRequest = function(method,url,async,callback) {
	this.url				=	url;
	this.method				= 	method;
	this.async				=	async;
	this.init				=	function() {	
...
feelview.XMLHttpRequest.prototype.setRequestHeader	= function(name,value) {
	this.requestHeaderName	= name;
	this.requestHeaderValue	= value;
}
feelview.XMLHttpRequest.prototype.onreadystatechange	= function() {
...
feelview.XMLHttpRequest.prototype.send = function(httpBody) {
...
feelview.TransformXmlXsl	= function(xmlUrl, xslUrl, callbackName,xmlMethod, httpBody) {
...
feelview.TransformXmlXsl.prototype.transform	=	function(args) {
	this.args		=	args || null;
	var context		=	this;
	if(typeof(this.xmlUrl.xml) != "undefined")
		this.xmlDom	= this.xmlUrl;
	else {
		var xmlhttpXML = new feelview.XMLHttpRequest(this.xmlMethod,this.xmlUrl,true,this.xmlCallback);
	...
	if(typeof(this.xslUrl.xml) != "undefined") {
		...
		var xmlhttpXSL = new feelview.XMLHttpRequest("GET",this.xslUrl,true,this.xslCallback);
...
feelview.TransformXmlXsl.prototype.xmlCallback	= function(xmlDom) {
	this.xmlDom = xmlDom;
	this.onDataReady();	
}
feelview.TransformXmlXsl.prototype.xslCallback	= function(xslDom) {
	this.xslDom = xslDom;
	this.onDataReady();
}

feelview.TransformXmlXsl.prototype.onDataReady = function() {
...
feelview.TransformXmlXsl.prototype.transformNode = function() {
	var result = null;
	try {
		if(window.ActiveXObject) {
			if(this.args == null) {
