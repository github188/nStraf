...
<head><title></title></head>
	...
	<script type="text/javascript"> 
		...
		function query(){
			...
			var actionUrl = "<%=request.getContextPath()%>/pages/totalPriceHist/totalPriceHistinfo!refresh.action?from=refresh&pageNum="+pageNum;
		...
		function setHTML(entry,entryInfo){
			...
			//html += '<td align="center">' +entryInfo["monthDate"] + '</td>';
			...
			html += '<td>' +entryInfo["workTitleFut"] + '</td>';
				if(entryInfo["workTitleHist"]!=entryInfo["workTitleFut"]&&entryInfo["workTitleHist"]!=""){
			
			    if(entryInfo["workTitleFut"]=="助理软件评测师"){
					html += '<td bgcolor="#00CC66">' +entryInfo["workTitleHist"] + '</td>';			   		
				}
				else if(entryInfo["workTitleFut"]=="初级软件评测师")
				{
            	 		if(entryInfo["workTitleHist"]!="助理软件评测师"){	
			   				html += '<td bgcolor="#00CC66">' +entryInfo["workTitleHist"] + '</td>';		
						}
						else
						{
							html += '<td bgcolor="#990000">' +entryInfo["workTitleHist"] + '</td>';		
                        }
                }
				else if(entryInfo["workTitleFut"]=="中级软件评测师")
				 {
            	 		 if(entryInfo["workTitleHist"]!="高级软件评测师"){	
			   				html += '<td bgcolor="#990000">' +entryInfo["workTitleHist"] + '</td>';	
						}
						else
						{
		 ...
		 {
			 	html += '<td>' +entryInfo["workTitleHist"] + '</td>';
		 }
            	
			
			html += '<td>' +entryInfo["updateName"] + '</td>';
			html += '<td>' +entryInfo["updateDateString"] + '</td>';
	  		html += '</tr>';
	 		<% k++;%>;
		...
		function del() {
			var aa=document.getElementsByName("chkList");
				...
				if(confirm('<s:text name="确认删除该记录吗？" />')) {
					var strUrl="/pages/totalPriceHist/totalPriceHistinfo!delete.action?ids="+idList;
		...
		function modify(){
			...
			if (j==0)
			 	...
			 	alert('<s:text name="operator.updateone" />')
			  	...
			  	var strUrl="/pages/totalPriceHist/totalPriceHistinfo!edit.action?ids="+itemId;
	...
	function add(){
		var resultvalue = OpenModal('/pages/totalPriceHist/totalPriceHistinfo!add.action','740,470,tmlInfo.addTmlTitle,tmlInfo');
		if(resultvalue!=null)
	  		query();
	}
	...
	function up()
	{
		var resultvalue = OpenModal('/pages/totalPriceHist/totalPriceHistinfo!up.action','740,470,tmlInfo.updateTitle,tmlInfo');
		if(resultvalue!=null)
	  		query();
	}
	...
	function refreshUp()
	{
		...
		function show(id) {
			var strUrl="/pages/totalPriceHist/totalPriceHistinfo!show.action?ids="+id;
 ...
 <body id="bodyid" leftmargin="0" topmargin="0"><br>
	...
	%>
 	<s:form name="reportInfoForm"  namespace="/pages/totalPriceHist" action="totalPriceHistinfo!list.action" method="post" >
	...
	<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000066" id=downloadList>
		<tr> 
			...
			<td nowrap width="8%" class="oracolumncenterheader"><div align="center">原岗位</div></td>
            <td nowrap width="8%" class="oracolumncenterheader"><div align="center">调整后岗位</div></td>
            <td nowrap width="5%" class="oracolumncenterheader"><div align="center">最近更新者</div></td>
            <td nowrap width="10%" class="oracolumncenterheader"><div align="center">最近更新日期</div></td>
		</tr>
		<tbody name="formlist" id="formlist" align="center"> 
  		<s:iterator value="toatlPriceHistList" id="tranInfo" status="row">
 		...
 		<tr id="tr" class="trClass1" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
            ...
            <td><div align="center">
				<s:property value="#row.count"/></font></div></td>
		 	<td><div>
	    ...
	    <td><s:property value="workTitleFut"/></td>
			
			
			
			<s:if test="workTitleHist!=workTitleFut" >
			
			    <s:if test='workTitleFut=="助理软件评测师"' >	
			   		<td bgcolor="#00CC66"><s:property value="workTitleHist"/></td>
				</s:if>
				 <s:elseif test='workTitleFut=="初级软件评测师"' >
            	 		 <s:if test='workTitleHist!="助理软件评测师"' >	
			   				<td bgcolor="#00CC66"><s:property value="workTitleHist"/></td>
						</s:if>
						<s:else>
                       	  <td bgcolor="#990000"><s:property value="workTitleHist"/></td>
                       </s:else>
                </s:elseif>
				
				<s:elseif test='workTitleFut=="中级软件评测师"' >
            	 		 <s:if test='workTitleHist!="高级软件评测师"' >	
			   				<td bgcolor="#990000"><s:property value="workTitleHist"/></td>
						</s:if>
						<s:else>
                       	  <td bgcolor="#00CC66"><s:property value="workTitleHist"/></td>
            ...
            <s:else>
            	<td><s:property value="workTitleHist"/></td>
            </s:else>
			
				  	
            <td><s:property value="updateName"/></td>
            <td><s:property value="updateDateString"/></td>
	    </tr>
		</s:iterator>  
 		</tbody> 
 	</table> 
