...
function toExport(){
	...
	var actionUrl = "";
	if(type=="0"){//导出广发数据
		actionUrl = "<%=request.getContextPath()%>/pages/attendance/attendanceAction!exportAttendanceData.action?startdate="+signTime;
		actionUrl += "&enddate="+signEndTime;
	}else{
		actionUrl = "<%=request.getContextPath()%>/pages/attendance/attendanceAction!exportAllAttendanceData.action?startdate="+signTime;
		actionUrl += "&enddate="+signEndTime;
	}
