...
<html>
	...
	function query(){
		...
		var actionUrl = "<%=request.getContextPath()%>/pages/analyse/blReguAction!refresh.action?from=refresh&pageNum="+pageNum;
		actionUrl += "&regulation="+regulation+"&beginDate="+beginDate+"&endDate="+endDate;
	...
	function setHTML(entry,entryInfo){
		...
		html += '<input type="checkbox" name="chkList" value="' + entryInfo["regulation"] + '" />';
		html += '</td>';
		html += '<td align="center">';
		html += '<a href=javascript:showTransInfo("' + entryInfo["regulation"] + '")>' + entryInfo["regulation"] + '</a>';
		html += '</td>';
	...
	function showTransInfo(regulation) {
		var beginDate =  document.getElementById("beginDate").value;
		var endDate =  document.getElementById("endDate").value;
		regulation += "|" + beginDate + "|" + endDate;
		var strUrl="/pages/watch/tranAbnoinfo!list.action?regulation="+regulation;
		//alert(accountNo);
		var features="700,500,watch.list.blRegu,transmgr";
		var resultvalue = OpenModal(strUrl,features);
	}  
	</script>
 
 <body id="bodyid" leftmargin="0" topmargin="0">
 	<s:form name="RegulationForm"  namespace="/pages/analyse" action="/pages/analyse/BlReguAction!list.action" method="post" >
		...
		<table width="100%" cellSpacing="0" cellPadding="0" class="selectTableBackground"> 
 		...
 		<td>
			...
			<table width="100%">
			  <tr>
			  	<td align="center"><s:text name="rule.list.regulation"/><s:text name="label.colon" /></td>
				<td><input name="regulation" type="text" id="regulation"  class="MyInput"> </td>
					...
					<tr>
						...
						<td width="11%">
	 					...
	 					<td width="83%" align="right">
							<div id="pagetag"> <tm:pagetag pageName="currPage" formName="RegulationForm" /></div>
						</td>
					</tr>
					</table>
				</td>
		...
		<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000066" id=downloadList>
			<tr class="oracolumncenterheader"> 
				...
				<td width="9%" style="cursor:hand"><s:text name="label.analyse.regulation" /></td>
				<td width="10%" style="cursor:hand"><s:text name="label.analyse.count" /></td>
			</tr>
			<tbody name="formlist" id="formlist">
				<s:iterator  value="regulationList" id="regulationAnalyse" status="row">
					...
					<tr id="tr" class="trClass1" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
						...
						<td align="center">
							<input type="checkbox" name="chkList" value='<s:property value="regulation" />' />
						</td>
						<td align="center">
							<a href='javascript:showTransInfo("<s:property value="regulation"/>")'><s:property value="regulation" /></a>
						</td>
