...
<form name="prjContractInfoForm" action="<%=request.getContextPath()%>/pages/prjContract/prjContractInfo!update.action" method="post">
			...
			<td>
					...
					<td class="input_tablehead">
						修改合同信息
			      		...
			      		<input name="prjContract.Id" type="hidden"  value='<s:property value="prjContract.Id"/>' >
			      		<tr>
							...
							<td  bgcolor="#FFFFFF" width="25%">
    			                ...
    			                <option value="<s:property value='username'/>" <s:if test='username==#request.prjContract.prjManager'>selected</s:if>><s:property value="username"/> </option>
    		                    </s:iterator>
    	                        </select>
							</td>
						...
						<tr>
							...
							<td  bgcolor="#FFFFFF" >
								<s:textfield name="prjContract.total" size="25%" id="total" onkeyup="this.value=this.value.replace(/[^0-9\.]+/,'')" onKeyDown="this.value=this.value.replace(/[^0-9\.]+/,'')"/>
							&nbsp;元
							</td>
							<td class="input_label2">
								<div align="center"> 
							...
							<td bgcolor="#FFFFFF" colspan="3">
								<s:textfield name="prjContract.payment" size="20%" id="payment" onkeyup="this.value=this.value.replace(/[^0-9\.]+/,'')" onKeyDown="this.value=this.value.replace(/[^0-9\.]+/,'')"/>
                           	&nbsp;元
                           	</td>
						</tr>
						...
						<tr>
							...
							<td bgcolor="#FFFFFF">
								...
								<option value="">&nbsp;<s:text name="notify.option"></s:text>&nbsp;</option>
	                               	<option value='新增' <s:if test="'新增'==prjContract.status">selected</s:if>> 新增 </option>
	                               	<option value='执行中' <s:if test="'执行中'==prjContract.status">selected</s:if>> 执行中</option>
	                               	<option value='意外中止' <s:if test="'新增'==prjContract.status">selected</s:if>> 意外中止 </option>
	                               	<option value='结束' <s:if test="'结束'==prjContract.status">selected</s:if>> 结束</option>
	                            </select> 
							</td>
							<td  class="input_label2" width="13%">
								<div align="center"> 
							...
							<td width="14%" bgcolor="#FFFFFF">
								<s:textfield name="prjContract.client" maxlength="50" size="20%" id="client"/>
                           	</td>
						</tr>
						...
						<tr>
							...
							<td bgcolor="#FFFFFF" >
								<s:textfield name="prjContract.fare" maxlength="20" size="20" id="fare"/>
							</td>
							<td class="input_label2">
								<div align="center">
							...
							<td  colspan="3" bgcolor="#FFFFFF">
								<input name="prjContract.startDate" type="text" id="startDate" class="MyInput" 
						...
						<tr>
							...
							<td bgcolor="#FFFFFF" >
								<input name="prjContract.endDate" type="text" id="endDate" class="MyInput" 
							...
							<td  colspan="3" bgcolor="#FFFFFF">
								<input name="prjContract.finishDate" type="text" id="finishDate" class="MyInput" 
						...
						<tr>
							...
							<td bgcolor="#FFFFFF" colspan="5">
								<div >
									<s:textarea name="prjContract.risk" onkeyup="limitLen(500,this)" cols="81" rows="3" id="risk"/>
								</div>									
							</td>
						</tr>
						<tr>
							...
							<td bgcolor="#FFFFFF" colspan="5">
								<div >
									<s:textarea name="prjContract.note" onkeyup="limitLen(500,this)" cols="81"  rows="3" id="note"/>
								</div>									
							</td>
						</tr>
					</table>
