...
<html>
	<head><title>repeat Regulation query</title></head>
	...
	<script type="text/javascript"> 
		function query(){
			...
			var actionUrl = "<%=request.getContextPath()%>/pages/rule/repeatRegulation!refresh.action?from=refresh";
			actionUrl += '&pageNum=' + pageNum + '&reguStatus=' + reguStatus;
		...
		function setHTML(entry,entryInfo){
			...
			html += '<tr align="center" id="tr" class="trClass<%=k%2 %>" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>'; 
				...
				html += '</td>';
				html += '<td>';
					html += '<a href=javascript:show("' + entryInfo["id"] + '")>' + entryInfo["applyId"] + '</a>';
				html += '</td>';
				html += '<td>' + entryInfo["repeatnum"] + '</td>';
				html += '<td>' + entryInfo["termNum"] + '</td>';
		...
		function del() {
			...
			if(confirm('<s:text name="rule.del.confirm" />')) {
			 	var strUrl="/pages/rule/repeatRegulation!delete.action?ids="+idList;
		...
		function add(){
			var resultvalue = OpenModal('/pages/rule/repeatRegulation!add.action','600,400,rule.addTitle,rule');
			if(resultvalue!=null)
		  		query();
		}
 ...
 <body id="bodyid" leftmargin="0" topmargin="0">
 	<s:form name="repeatRegulationForm"  namespace="/pages/rule" action="/pages/rule/repeatRegulation!query.action" method="post" >
					...
					<tr>
						...
						<td width="11%">
 						...
 						<td width="83%" align="right">
							<div id="pagetag"> 
								<tm:pagetag pageName="currPage" formName="repeatRegulationForm" />
							</div>
						</td>
					</tr>
				</table>
		...
		<table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000066" id="downloadList">
			<tr class="oracolumncenterheader"> 
				...
				<td width="9%" style="cursor:hand"><s:text name="rule.applyId" /></td>
				<td width="10%" style="cursor:hand"><s:text name="rule.repeatnum" /></td>
				<td width="8%" style="cursor:hand"><s:text name="rule.list.termNum" /></td>
			...
			<tbody name="formlist" id="formlist"> 
  			<s:iterator  value="repeatRegulationList" id="repeatRegulation" status="row">
  			<s:if test="#row.odd == true"> 
 			<tr align="center" class="trClass0" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
 			</s:if><s:else>
 			<tr align="center" class="trClass1" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
 			</s:else>
				<td align="center">
				...
				<!-- <td><tm:dataDir beanName="repeatRegulation" property="regulationStatus" path="ruleMgr.blMgr" /></td> -->
				<td><tm:dataDir beanName="repeatRegulation" property="reguStatus" path="ruleMgr.reguDeliverStatus" /></td>
