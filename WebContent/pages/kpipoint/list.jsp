...
<script type="text/javascript"> 
		   ...
		   function document.onkeydown()
		    {	
	 		...
	 		function query(){			
				...
				Num2 = 0;
				pageNumTmp = pageNum;
			var actionUrl = "<%=request.getContextPath()%>/pages/kpipoint/kpipointinfo!refresh.action?from=refresh&pageNum="+pageNum;
		...
		function setHTML(entry,entryInfo){
			...
			html += '<td>' +entryInfo["subsum_1"] + '</td>';
			html += '<td>' +entryInfo["subsum_2"] + '</td>';
		...
		function del() {
			...
			if(confirm('<s:text name="确认删除该记录吗？" />')) {
				//alert("dddddd");
			 	var strUrl="/pages/kpipoint/kpipointinfo!delete.action?ids="+idList;
		...
		function modify(){
			...
			if (j==0)
			 	...
			 	alert('<s:text name="operator.updateone" />')
			  	...
			  	var strUrl="/pages/kpipoint/kpipointinfo!edit.action?ids="+itemId;
	...
	function add(){
    	...
    	var resultvalue = OpenModal('/pages/kpipoint/kpipointinfo!add.action','1100,300,tmlInfo.addTmlTitle,tmlInfo');
		  if(resultvalue!=null)
	  		 query();
	}
	function up(){
      var aa=document.getElementsByName("chkList");
			...
			if (j==0)
				...
				if(confirm('<s:text name="确认提交该记录吗？" />')) {
					var strUrl="/pages/kpipoint/kpipointinfo!up.action?ids="+itemId;
	...
	function UpAuditing(){
			...
			if (j==0)
				...
				if(confirm('<s:text name="确认提交审核该记录吗？" />')) {
					var strUrl="/pages/kpipoint/kpipointinfo!UpAuditing.action?ids="+itemId;
			  		var features="600,300,tmlInfo.updateTitle,tmlInfo";
					var resultvalue = OpenModal(strUrl,features);
				    query();
				}
		...
		function show(id) {
			var strUrl="/pages/kpipoint/kpipointinfo!show.action?ids="+id;
	...
	var re=/^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012]))|(((1[6-9]|[2-9]\d)\d{2})-0?2)|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2))$/g; 
  	var thisDate = document.getElementById(monthVal).value.trim();
		...
		function search()
		{
			var url="kpipointinfo!queryNames.action";
			var params={groupname:$("#groupname").val()};
			if($("#levelHidden").val()=='1')
			{
				var params={groupname:$("#groupNameHidden").val()};
			}
 ...
 <body id="bodyid" leftmargin="0" topmargin="0"><br>
 	<s:form name="reportInfoForm"  namespace="/pages/kpipoint" action="kpipointinfo!list.action" method="post" >
...
<tr> 
      ...
      <td nowrap width="10%" class="oracolumncenterheader"><div align="center">出勤日</div></td>
      <td nowrap width="10%" class="oracolumncenterheader"><div align="center">加班时长</div></td>
      ...
      <td nowrap width="20%" class="oracolumncenterheader"><div align="center">最近更新日期</div></td>
		...
		<tbody name="formlist" id="formlist" align="center"> 
 		...
 		<tr id="tr" class="trClass1" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
			...
			<td align="center"><s:property value="subsum_1"/></td>
            <td align="center"><s:property value="subsum_2"/></td>
 ...
 if($("#levelHidden").val()=='1')
 {
	htmlshow = "<input name='groupName' type='text' id='groupName' readonly value='" + $("#groupNameHidden").val().trim() + "' style='background-color:#FFFFFF; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; color:#000000'/>";
	$("#groupDiv").html(htmlshow);
	htmlshow = '<select name="username" id="username" style="width:163px;"></select>';
 ...
 else if($("#levelHidden").val()=='2')
 {
 	htmlshow = "<input name='groupName' type='text' id='groupName' readonly value='" + $("#groupNameHidden").val().trim() + "' style='background-color:#FFFFFF; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; color:#000000'/>";
	$("#groupDiv").html(htmlshow);
	htmlshow = "<input name='username' type='text' id='username' readonly value='" + $("#usernameHidden").val().trim() + "' style='background-color:#FFFFFF; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; color:#000000'/>"
	$("#userDiv").html(htmlshow);
 }
 else
 {
 	htmlshow = '<select name="groupname" style="width:163px;" id="groupname" onChange="search()" >	'+	
			   '<option value="全选">全选</option>'+
              '<option value="质量管理组">质量管理组</option>'+
              '<option  value="应用软件测试组">应用软件测试组</option>'+
              '<option value="基础软件测试组">基础软件测试组</option>'+
