...
<%
	String indexvalue=(String)request.getParameter("indexvalue");
%>
<html>
<head></head>
...
function formsubmit(){
	   ...
	   if(navigator.userAgent.indexOf("MSIE") != -1){//ie浏览器
		   ...
		   window.close();
			...
			parent.projectSelectPeople(usergroups,"<%=indexvalue%>");
	   }
}
...
$(function(){
	$("#deptname").change(function(){
		...
		var url = "<%=request.getContextPath()%>/pages/project/project!getUserProjectByDeptName.action";
		$.ajax({
			url:url,
			datatype:"json",
			type:"post",
...
<input type="hidden" id="projectId" name="projectId" value="<s:property value='#request.projectId'/>"/>
 ...
 <tr>
  <td align="left" width="270" style="padding:2px 5px 2px 8px;line-height:20px;">
   ...
   <select id="outgroup" name="OutGroup" size="15" style="width:180px;height:250px" multiple ondblclick="MoveSelection(document.frm.OutGroup,document.frm.usergroups)">
      <s:iterator value="#request.notinusers" id="user">
      	<s:if test="#user != null">
  ...
  <td align="left" style="padding:2px 10px 2px 20px;line-height:20px;">
   ...
   <select id="usergroups" name="usergroups" size="15" style="width:180px;height:250px" multiple ondblclick="MoveSelection(document.frm.usergroups,document.frm.OutGroup)">
    	<s:iterator value="#request.inusers" id="user2">
