...
<script type="text/javascript">
   ...
   if(flag){
	...
	/**--------------------------------其他信息-------------------------------------------**/
 	function addAgain(){
		if($("#otherInfoContainer").children().size()<=0){
			...
			$("#otherInfoContainer").children("table:last").after(getOtherInfoTable());
		}
		//重新对checkbox的序号编号
		renameCheckboxSpan();
	}
	...
	function delAgain(){
		//被选中的checkbox
		var checked=0;
		$("div[name='otherInfoCheckBoxDiv']").find("input[type='checkbox']").each(function(){
	...
	/**--------------------------------跟进信息-------------------------------------------**/
 	function addAgain2(){
		if($("#followContainer").children().size()<=0){
			...
			$("#followContainer").children("table:last").after(getFollowTable());
		}
		//重新对checkbox的序号编号
		renameCheckboxSpan2();
		resetLastFollowDate();
	...
	function delAgain2(){
		//被选中的checkbox
		var checked=0;
		$("div[name='followCheckBoxDiv2']").find("input[type='checkbox']").each(function(){
			 if($(this).attr('checked')==true||$(this).prop('checked')==true){
			 	checked++;
			 }
		});
	...
	function getFollowTable(){
		...
		html+='					<td bgcolor="#FFFFFF" style="width:23%">';
		html+='						<input type="text" class="CalendarInput" name="followList_followDate" onchange="resetLastFollowDate()" value="'+getNowTime()+'"  style="width:90%" value=""/>';
		html+='					</td>';
	...
	function resetLastFollowDate(){
		...
		$("#followContainer").find("input[name='followList_followDate']").each(function(){
			var thisDate=$(this).val();
			if(thisDate!=''&&compareCalendar(thisDate,lastFollowDate)==1){
...
<body id="bodyid">
<form name="prjChanceInfoForm" action="<%=request.getContextPath()%>/pages/prjchance/prjChanceInfo!save.action" method="post">
										...
										<td bgcolor="#E6E6E6">
											<table width="100%" align="center" cellPadding="1"
												...
												<tr>
													...
													<td bgcolor="#FFFFFF" colspan="5">
														<textarea name="followList_followContent" onkeyup="limitLen(2000,this)" cols="84" rows="2" ><s:property value="prjChance.note" /> </textarea>
													</td>
												</tr>
											</table>
										</td>
