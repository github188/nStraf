...
<html>
	...
	<script type="text/javascript">
		...
		function save(){
			...
			if(document.getElementById("performance.month_date").value.trim()=="")
			{
				alert("请输入月份");
				return;
			}
	...
	var re=/^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012]))|(((1[6-9]|[2-9]\d)\d{2})-0?2)|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2))$/g; 
  	var thisDate = document.getElementById(monthVal).value.trim();
		...
		function startMonthHdl()
		{
			...
			else if(document.getElementById("performance.month_date").value.trim() != "")
			{
				if(validateInputStart("performance.month_date"))
				{
					dayFlag = true;
				}
				else if(validateInputStartMonth("performance.month_date"))
				{
					monthFlag = true;
				}
				else
				...
				if(dayFlag)
				{
					var str = document.getElementById("performance.month_date").value.split("-");
					if(str[1]!="10" && str[1]!="11" && str[1]!="12")
					{
						document.getElementById("performance.month_date").value = parseInt(str[0],10) + "-" + "0" + parseInt(str[1],10);
					}
					else
					{
						document.getElementById("performance.month_date").value = parseInt(str[0],10) + "-" + parseInt(str[1],10);
					}
				}
				checkMonthStartFlag = true;
				return true;
	...
	<body id="bodyid" leftmargin="0" topmargin="10">
		<form name="reportInfoForm"
			action="<%=request.getContextPath()%>/pages/performance/performanceinfo!save.action"
			method="post">
			<table width="90%" align="center" cellPadding="0" cellSpacing="0">
				<tr>
					<td>
					  <fieldset class="jui_fieldset" width="90%">
							<legend>新增月度考核</legend>
		  ...
		  <div align="center">
								  ...
								  <font color="#FF0000">*</font>										</div>									</td>
									<td bgcolor="#FFFFFF" width="80%"  onClick="startMonthHdl()"><input name="performance.month_date" type="text"
