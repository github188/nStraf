...
<script type="text/javascript">
	...
	function showreason(o){
		...
		function save(){
			...
			var approveContent = document.getElementById("approveContent").value;
			if(approveStatus=="0"){
				if(approveContent==""){
					alert("请输入审核结论");
					document.getElementById("approveContent").focus();
					return ;
				}
			}else if(approveContent==""){
				document.getElementById("approveContent").value="通过";
			}
		/*	if(document.getElementById("reason").value.length>200){
				alert("打回原因字数限制在200字以内，您输入的字数为"+document.getElementById("reason").value.length+";请重新输入！");
			...
			var flag="false";
			if(Number($("#approveSum").val())!=Number($("#sum").val()) && approveStatus=="1"){
	...
	$(function(){
		...
		$(".removeTr").click(function(){
			...
			$("#approveSum").val((Number(travelSum)+Number($(costSumTd[12]).html())).toFixed(2));
			var sum = $("#sum").val();
			if(sum!=""){
		...
		$(".modifyTr").click(function(){
			...
			if(btnValue=="确认"){
				...
				$("#approveSum").val((Number(travelSum)+Number($(costSumTd[i]).html())).toFixed(2));
				var sum = $("#sum").val();
				if(sum!=""){
					var showsum=fmoney(sum,2);
					$("#showsum").val(showsum);
		...
		$(".removeTr1").click(function(){
			...
			$("#approveSum").val((Number(costSum)+Number($(travelSumTd[3]).html())+Number($(travelSumTd[4]).html())+Number($(travelSumTd[5]).html())).toFixed(2));
			var sum = $("#sum").val();
			if(sum!=""){
				var showsum=fmoney(sum,2);
				$("#showsum").val(showsum);
		...
		$(".modifyTr1").click(function(){
			...
			if(btnValue=="确认"){
				...
				$("#approveSum").val((Number(costSum)+Number(travelSum)).toFixed(2));
				var sum = $("#sum").val();
				if(sum!=""){
					var showsum=fmoney(sum,2);
					$("#showsum").val(showsum);
...
<body id="bodyid" leftmargin="0" topmargin="10">
	<form name="expenseInfoForm" id="expenseInfoForm"
		action="<%=request.getContextPath()%>/pages/expense/expenseInfo!finAudit.action"
		method="post">
			...
			<tr>
				<td>
					<table width="100%" align="center" border="0" cellspacing="1" cellpadding="1" class="input_table">
						<input name="expenseId" id="expenseId" type="hidden" value='<s:property value="expenseAccount.id"/>'>
						<tr>
							<td class="input_tablehead">报销财务审核</td>
						</tr>
					</table>
					<table width="95%" class="input_table">
						<tr>
							<td colspan="2">
								...
								报销状态：财务审核中&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input name="expenseAccount.status" id="status" readonly="true" type="hidden" value='2'>
						...
						<tr>
							...
							<td bgcolor="#FFFFFF">
								<textarea name="expenseAccount.note" cols="73" rows="4" id="note" style="width:100%" readonly="readonly"><s:property value="#request.expenseAccount.note"/></textarea>
							</td>
						</tr>
						<tr>
							<td class="input_label2">审核结果</td>
							<td bgcolor="#FFFFFF">
								<select name="approveStatus" width="15%" id="approveStatus" onChange="approveStatusChange()">
               						<option value="1" >财务审核通过，已发款</option>
									<option value="0">财务审核不通过</option>
                				</select>
							</td>
						</tr>
						<tr>
							...
							<td bgcolor="#FFFFFF">
								<input type="text" name="expenseAccount.approveSum" id="approveSum" maxLength="7" onkeyup="this.value=this.value.replace(/[^0-9\.]+/,'')" onKeyDown="this.value=this.value.replace(/[^0-9\.]+/,'')"  value='<s:property value="#request.expenseAccount.sum"/>'/>元
							</td>
						</tr>
						<tr>
						...
						<tbody name="li" id="li" name="formlist" id="formlist"
							...
							<s:iterator value="#session.expenselist" id="costDetail"
								...
								<tr style="border: 1">
									...
									<td align="center" title='<s:property value="place" />'><s:property value="place" /></td>
									...
									<td align="center"><input name="detailId" type="hidden"
										...
										type="button" value="修改" style="width:36px;margin-left:-5px;"/> <input class="removeTr"
						...
						<tbody name="pslist" id="pslist" align="center">
							<s:iterator value="#session.savePrjSumList" id="prjsumlist" status="row">
								<tr id="prjsumtr<s:property value='#row.index'/>">
									<td align="center" width="5%;"><input type="checkbox" name="chkList" value="<s:property value='#row.index'/>"/></td>
									<td align="right" width="10%;">项目名称</td>
									<td align="left" width="40%;">
