	...
	if(!change()){
	 ...
	 function change(){
		...
		if(!filter(file.value)){
			alert("您上传的文件类型不被支持，本系统只支持Excel文件");
			return false;
		}
		return true;
	} 
	 ...
	 function filter(name){
		...
		if(lastvalue!="XLS"&&lastvalue!="XLSX"&&lastvalue!="ET"&&lastvalue!="XLT"&&lastvalue!="ETT"){
			return false
		}
		return true;
	 }
...
<body>
	<s:form action="/pages/expense/expenseInfo!addImportData.action" method="post" enctype="multipart/form-data">
      <br>
      <table border="0" width="100%" cellspacing="0" cellpadding="0">
      <input name="uuid" type="hidden"  value='<%=request.getParameter("uuid") %>' >
      <input name="note" type="hidden"  value='<%=request.getParameter("note") %>' >
      <input name="updatemanid" type="hidden"  value='<%=request.getParameter("updatemanid") %>' >
      	<tr>
        ...
        <tr>
          ...
          <s:file name="file" id="file" cssStyle="MyButton" onchange="change(this);"></s:file>
          </td>
          <td width="1%"></td>
        </tr>
