...
<head><title>workReport query</title></head>
 ...
 <body id="bodyid" leftmargin="0" topmargin="0"><br>
 	...
 	<table width="100%" cellSpacing="0" cellPadding="0" class="selectTableBackground"> 
 		...
 		<td>
              ...
              <tr>
				...
				<td width="25%"> 
					...
					<select  name="groupName1" style="width:163px;" id="groupName1" onChange="search()" >
              				...
              				<option value="循环机芯组">循环机芯组</option>
										<option  value="存款机芯组">存款机芯组</option>
										<option value="通用模块组">通用模块组</option>
										<option  value="清分机组">清分机组</option>
										<option  value="出钞机芯组">出钞机芯组</option>
              		</select > 
				</td>
                ...
                <s:elseif test="#session.level==1">
				<td width="25%"> 
				<input name="usernameHidden" type="hidden" id="usernameHidden"  value='<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getUsername() %>'/>
					<input name="groupNameHidden" type="hidden" id="groupNameHidden"  value='<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getGroupName() %>'/>
					<input name="groupName1" type="text" id="groupName1"  value='<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getGroupName() %>' readonly style="width:163px; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; background:none; border-width:thin"/>
			...
			<s:else>
				...
				<input name="usernameHidden" type="hidden" id="usernameHidden"  value="<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getUsername() %>"/>
					<input name="groupNameHidden" type="hidden" id="groupNameHidden"  value="<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getGroupName() %>"/>
					<input name="groupName1" type="text" id="groupName1"  value='<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getGroupName() %>' readonly style="width:163px; border-width:thin; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; background:none"/>
              		
				</td>
              	<td align="center">员工姓名：</td>
				<td >
                	<input name="username" type="text" id="username"  value="<%=((cn.grgbanking.feeltm.login.domain.UserModel)session.getAttribute("tm.loginUser")).getUsername() %>" readonly style="width:163px; border-width:thin; border-right-style:none; border-bottom-style:none; border-left-style:none; border-top-style:none; background:none"/>
				</td>
			</s:else>
	...
	<script type="text/javascript">
		...
		var levelHidden = parseInt(document.getElementById("levelHidden").value);
		if(levelHidden==0 || levelHidden==1)
		{
			search();
		}
	</script>
