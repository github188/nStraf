...
function init(){
	...
	var em = document.getElementsByTagName('input');
	for(var  i=0;i<em.length;i++){
		  if(em[i].type=="checkbox" && em[i].id=="deptSelect"){
			   var text=em[i].value.split(",")[0];
			   text=text.split(":")[0];
			   for(var j=0;j<studentids.split(",").length;j++){
				   if(studentids.split(",")[j]==text && text!=""){
					   em[i].checked=true;
				   }
			   }
		   }
...
<body id="menu" onload="init();">
 ...
 <s:iterator value="#request.deptUser" id="dept" status="status">
	  ...
	  <input type="checkbox" value="" onclick="SelAll(this,'<s:property value="key"/>')" id="deptSelect"> 
	  <s:property value="#request.deptmaMap[key]"></s:property></a></h2>
	  <ul class="no">
	  	<s:iterator value="value">
		   <a href="javascript:void(0)"><input id="deptSelect" name="<s:property value="key"/>" type="checkbox" value="<s:property value="userid"/>:<s:property value="username"/>"><s:property value="username"/></a>
	   </s:iterator>
	  </ul>
  </s:iterator>
 </span>
 <div align="center">
 <input type="hidden" id="studentids" name="studentids" value='<s:property value="#request.studentids" />'/>
