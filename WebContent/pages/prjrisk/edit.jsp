...
<script type="text/javascript">
	...
	function save() {
		...
		if (document.getElementById("handleterm").value.trim() == "") {
			alert("请输入计划完成日期");
			return;
		}
...
<body id="bodyid" leftmargin="0" topmargin="10">
					...
					<table class="input_table">
						...
						<tr>
							...
							<td bgcolor="#FFFFFF" width="26%"><input
								...
								value='<s:property value="prjRisk.createman"/>' readonly
						...
						<tr>
							...
							<td width="14%" bgcolor="#FFFFFF"><input
								name="prjRisk.status" type="text" id="status" maxlength="10"
								size="10" value='新建' readonly class="input_readonly"></td>
						...
						<tr>
							...
							<td colspan="3" bgcolor="#FFFFFF"><tm:tmSelect
									name="prjRisk.prjname" id="prjname" selType="projectName" style="width:240px" /></td>
							<input name="stat1" type="hidden" id="stat1"
								value='<s:property value="prjRisk.prjname"/>' />
							...
							<input type='hidden' id="st2" name="st2"
								value="<s:property value='prjRisk.type'/>" />
							<s:if test="prjRisk.type!=null&&!prjRisk.type.equals('')">
								<script language="javascript">
									var type = document.getElementById("st2").value;
									type = decodeURI(type);
						...
						<tr>
							...
							<input type='hidden' id="st3" name="st3"
								value="<s:property value='prjRisk.pond'/>" />
							<s:if test="prjRisk.pond!=null&&!prjRisk.pond.equals('')">
								<script language="javascript">
									var pond = document.getElementById("st3").value;
									pond = decodeURI(pond);
									document.getElementsByName("pond")[0].value = pond;
								</script>
							</s:if>
							...
							<input type='hidden' id="st4" name="st4"
								value="<s:property value='prjRisk.urgent'/>" />
							<s:if test="prjRisk.urgent!=null&&!prjRisk.urgent.equals('')">
								<script language="javascript">
									var urgent = document.getElementById("st4").value;
									urgent = decodeURI(urgent);
									document.getElementsByName("urgent")[0].value = urgent;
								</script>
							</s:if>
						</tr>
						<tr>
							...
							<td bgcolor="#FFFFFF" colspan="5">
								<div>
									<textarea cols="74" rows="3" name="prjRisk.riskdesc"
										id="riskdesc" style="width:100%"><s:property value="prjRisk.riskdesc" /></textarea>
								</div>
							</td>
						</tr>

						<tr>
							<td class="input_label2">建议方案 <font color="#FF0000">*</font>
							</td>
							<td bgcolor="#FFFFFF" colspan="5"><textarea cols="74"
									rows="3" name="prjRisk.suggest" id="suggest" style="width:100%"><s:property
										value="prjRisk.suggest" /></textarea></td>
						</tr>
						<tr>
							<td width="13%" class="input_label2">行动计划</td>
							<td bgcolor="#FFFFFF" colspan="5"><textarea
									name="prjRisk.plan" cols="74" rows="3" disabled id="plan" style="width:100%"><s:property
						...
						<tr>
							...
							<!-- 
								...
								<td bgcolor="#FFFFFF"><input name="prjRisk.factdate"
									type="text" disabled  id="factdate" class="input_readonly"
									value='<s:date name="prjRisk.factdate" format="yyyy-MM-dd"/>'></td>
								 -->
							<td class="input_label2" width="9%">
								<div align="center">处理者</div>
							</td>
							<td width="26%" bgcolor="#FFFFFF" colspan="3">><input
								name="prjRisk.handleman" type="text" disabled
								class="input_readonly" id="handleman"
								value='<s:property value="prjRisk.handleman"/>' size="20"
								maxlength="10"></td>
						</tr>
						<!-- 
							<tr>
								<td align="center" class="input_label2">解决情况</td>
								<td bgcolor="#FFFFFF" colspan="5"><textarea
										name="prjRisk.fruit" cols="74" rows="3" disabled 
										id="fruit"><s:property value="prjRisk.fruit" /></textarea></td>
							</tr> -->
						<input type="hidden" name="prjRisk.userid" id="userid"
							value="<s:property value='prjRisk.userid'/>" />
						<input type="hidden" name="prjRisk.deptname" id="deptname"
							value="<s:property value='prjRisk.deptname'/>" />
						<input type="hidden" name="prjRisk.groupname" id="groupname"
							value="<s:property value='prjRisk.groupname'/>" />
					</table>
				</td>
			</tr>
	...
	<script type="text/javascript">
		var szPrjName = document.getElementById("stat1").value;
		szPrjName = decodeURI(szPrjName);
		document.getElementById("prjname").value = (szPrjName.trim() == "" ? "0"
				: szPrjName.trim());
	</script>
</body>
</html>
