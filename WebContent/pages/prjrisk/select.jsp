...
function init(){
	...
	for(var  i=0;i<em.length;i++){
		  if(em[i].type=="radio"){
			   var text=em[i].value.split(",")[0];
			   text=text.split(":")[1];
			   if(text==inman && type=="inman"){
				   em[i].checked=true;
			   }
			   if(text==useman && type=="useman"){
				   em[i].checked=true;
			   }
		   }
	}
...
<body id="menu"> 
	<h1 onClick="javascript:ShowMenu(this,0)">
		<a href="javascript:void(0)">按部门选择</a></a>
	</h1>
	<span class="no"> <s:iterator value="#request.deptUser"
			id="dept" status="status">
			<h2
				onClick="javascript:ShowMenu(this,<s:property value="#status.index"/>)">
				<a href="javascript:void(0)">&nbsp;&nbsp; <s:property
						value="#request.deptmaMap[key]"></s:property></a>
			</h2>
			<ul class="no">
				<s:iterator value="value">
					<a href="javascript:void(0)">&nbsp;&nbsp; <input
						name="<s:property value="key"/>" type="radio"
						value="<s:property value="userid"/>:<s:property value="username"/>">
						<s:property value="username" />(<s:property value="userid"/>)</a>
				</s:iterator>
			</ul>
		</s:iterator>
	</span>
	<div align="center">
		...
		<input type="hidden" id="inman" value='<s:property value="#request.inman" />'/>
		<input type="hidden" id="useman" value='<s:property value="#request.useman" />'/>
