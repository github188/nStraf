...
<script type="text/javascript"> 
		   ...
		   function document.onkeydown()
		    {	
	 		...
	 		function query(){			
				...
				Num2 = 0;
				pageNumTmp = pageNum;
			var actionUrl = "<%=request.getContextPath()%>/pages/dailydeed/dailydeedinfo!refresh.action?from=refresh&pageNum="+pageNum;
		...
		function setHTML(entry,entryInfo){
			...
			html += '<td>' +entryInfo["subsum1"] + '</td>';
			html += '<td>' +entryInfo["subsum2"] + '</td>';
		...
		function del() {
			...
			if(confirm('<s:text name="确认删除该记录吗？" />')) {
				//alert("dddddd");
			 	var strUrl="/pages/dailydeed/dailydeedinfo!delete.action?ids="+idList;
		...
		function modify(){
			...
			if (j==0)
			 	...
			 	alert('<s:text name="operator.updateone" />')
			  	...
			  	var strUrl="/pages/dailydeed/dailydeedinfo!edit.action?ids="+itemId;
	...
	function add(){
    	...
    	var resultvalue = OpenModal('/pages/dailydeed/dailydeedinfo!add.action','1100,300,tmlInfo.addTmlTitle,tmlInfo');
		  if(resultvalue!=null)
	  		 query();
	}
	function up(){
      var aa=document.getElementsByName("chkList");
			...
			if (j==0)
				...
				if(confirm('<s:text name="确认提交该记录吗？" />')) {
					var strUrl="/pages/dailydeed/dailydeedinfo!up.action?ids="+itemId;
	...
	var re=/^((((1[6-9]|[2-9]\d)\d{2})-(0?[13578]|1[02]))|(((1[6-9]|[2-9]\d)\d{2})-(0?[13456789]|1[012]))|(((1[6-9]|[2-9]\d)\d{2})-0?2)|(((1[6-9]|[2-9]\d)(0[48]|[2468][048]|[13579][26])|((16|[2468][048]|[3579][26])00))-0?2))$/g; 
  	var thisDate = document.getElementById(monthVal).value.trim();
		...
		function search()
		{
			var url="dailydeedinfo!queryNames.action";
 ...
 <body id="bodyid" leftmargin="0" topmargin="0"><br>
 	<s:form name="reportInfoForm"  namespace="/pages/dailydeed" action="dailydeedinfo!list.action" method="post" >
...
<tr> 
      ...
      <td nowrap width="6%" class="oracolumncenterheader"><div align="center">月份</div></td>     
       <td nowrap width="8%" class="oracolumncenterheader"><div align="center">上级评分</div></td>
       <td nowrap width="8%" class="oracolumncenterheader"><div align="center">上上级评分</div></td>   
      ...
      <td nowrap width="15%" class="oracolumncenterheader"><div align="center">最近更新日期</div></td>
		...
		<tbody name="formlist" id="formlist" align="center"> 
 		...
 		<tr id="tr" class="trClass1" oriClass="" onMouseOut=TrMove(this) onMouseOver=TrMove(this)>
            ...
            <td align="center"><s:property value="subsum1"/></td>
            <td align="center"><s:property value="subsum2"/></td>
