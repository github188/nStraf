...
只需要date.jsp,datetime.jsp引用
**/
/*
//在date.jsp,datetime.jsp里面定义以下跟国际化相关的变量
...
function newCalendar(eltName,type) {
      ...
      var parseYear = parseIntNum(displayYear + '');
			...
			if(type==1){
       newCal = new Date(parseYear,displayMonth,1,parseHours,parseMinutes,displaySeconds);
			} else {
  ...
  function makeDaysGrid(startDay,day,intDaysInMonth,newCal,eltName,type) {
     ...
     daysGrid += '<input type=text id=year size=4  maxlength=4 style="BACKGROUND-color:transparent;border:0;color:red;font:9pt" value='+ year;
	 daysGrid += ' onkeypress=\"return dtp_KeyFilter(\'number\');\" onfocus=\"return dtp_focus(\'year\');\" onblur=\"setYear(\'' +eltName+'\',\'' + type + '\');\"><font color=red>'+yearLabel+'</font>'; 
     daysGrid += '<a style="cursor:hand" onClick="incYear(1,\'' + eltName + '\', \'' +type+'\')"><font size=1>&#9658;</font> </a>';
     daysGrid += '<a style="cursor:hand" onClick="incYear(10,\'' + eltName + '\', \'' +type+'\')"><font size=1>&nbsp;&nbsp;&#9658;&#9658;</font></a>';
	 daysGrid += '&nbsp;&nbsp;&nbsp;&nbsp;'
     //daysGrid += '<a title=Close href="javascript:hideElement(\'' + eltName + '\')">x</a>';
     daysGrid += '<a style="cursor:hand" onClick="incMonth(-1,\'' + eltName + '\', \'' +type+'\')"><font size=1>&#9668;</font> </a>';
     daysGrid += '<font color=red><select id=month onchange=\"setMonth(\'' + eltName + '\', \'' +type+'\')" style="font-size:9pt;color:red" >';
   ...
   if (dayOfMonth <= 0) {
	 ...
	 daysGrid += '><a style="cursor:hand" onMouseDown="selectDay(' + dayOfMonth + ',\'' + eltName + '\', \'' +type+'\')" ';     
...
function init(eltName,type)
{
  ...
  if (displayDivName && displayDivName != eltName) hideElement(displayDivName);
	...
	if(oldValue.split("-").length==2)
	{
		oldValue = oldValue + "-01";
	}
     //oldValue = "";     
  if(oldValue == "")
	{
